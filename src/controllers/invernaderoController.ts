...
export class InvernaderoController {
  // Aquí mantengo las funciones que tenías, pero las limpio para que no generen errores.
  ...
  // ✅ Nuevos métodos para cambiar el estado del invernadero
  public static async inactivarInvernadero(req: Request, res: Response): Promise<void> {
    try {
      const { id } = req.params;
      const [updated] = await Invernadero.update({ estado: 'inactivo' }, {
        where: { id_invernadero: id }
      });
      if (updated) {
        res.status(200).json({ message: 'Invernadero inactivado exitosamente.' });
      } else {
        res.status(404).json({ message: 'Invernadero no encontrado o ya estaba inactivo.' });
      }
    } catch (error: any) {
  ...
  public static async activarInvernadero(req: Request, res: Response): Promise<void> {
    try {
      const { id } = req.params;
      const [updated] = await Invernadero.update({ estado: 'activo' }, {
        where: { id_invernadero: id }
      });
      if (updated) {
        res.status(200).json({ message: 'Invernadero activado exitosamente.' });
      } else {
        res.status(404).json({ message: 'Invernadero no encontrado o ya estaba activo.' });
      }
    } catch (error: any) {
  ...
  public static async mantenimientoInvernadero(req: Request, res: Response): Promise<void> {
    try {
      const { id } = req.params;
      const [updated] = await Invernadero.update({ estado: 'mantenimiento' }, {
        where: { id_invernadero: id }
      });
      if (updated) {
        res.status(200).json({ message: 'Invernadero puesto en mantenimiento exitosamente.' });
      } else {
        res.status(404).json({ message: 'Invernadero no encontrado o ya estaba en mantenimiento.' });
      }
    } catch (error: any) {
      console.error('Error al poner en mantenimiento el invernadero:', error);
      res.status(500).json({ error: 'Error interno del servidor.', details: error.message });
    }
  }
}
